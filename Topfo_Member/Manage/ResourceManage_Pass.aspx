<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  AutoEventWireup="true" CodeFile="ResourceManage_Pass.aspx.cs" Inherits="Manage_ResourceManage_Pass" Title="Untitled Page" %>
...
var kong;
//绑定内容
function Hand(stat,page,ec,InfoType,title,name)
{
kong=ec;
...
}else
{
   ...
   $.ajax({type:"get",url:"Ajax.aspx",data:"name="+name+"&InfoType="+InfoType+"&Title="+title+"&Status="+stat+"&Type=1&Page="+page,success:function(msg) 
   { 
...
//绑定分页
function PageIndex(stat,page,InfoType,title,name)
{
   $.ajax({type:"get",url:"Ajax.aspx",data:"name="+name+"&InfoType="+InfoType+"&Title="+title+"&Status="+stat+"&Type=2&Page="+page,success:function(msg) 
   { 
...
var d = document.getElementById("<%=ddlInfoType.ClientID %>");//根据DropDownList的客户端ID获取该控件
 ...
 var title=document.getElementById("ctl00_ContentPlaceHolder1_txtTitle").value;
  $.ajax({type:"get",url:"Ajax.aspx",data:"name="+name+"&InfoType="+type+"&Title="+title+"&Type=3",success:function(msg) 
   { 
      var a=msg.split('$');
      pass.innerHTML=a[1];
      statu.innerHTML=a[0];
...
var name=document.getElementById("ctl00_ContentPlaceHolder1_LgName").value;
 ...
 var type = d.options[d.selectedIndex].value;//获取DropDownList当前选中值
 var title=document.getElementById("ctl00_ContentPlaceHolder1_txtTitle").value;
   Hand(stat,page,cd,type,title,name);
   PageIndex(stat,page,type,title,name);
   Nopass();
}
//模糊查询
function tt()
...
function SumDel()
{
i(document.all(cbxSelect).length<0)
{
   alert("请选择您要删除的数据");
   return false;
}
...
<input type="hidden" id="LgName" runat="server" />
<div class="member-right">
    <div class="publication">
      <h1><span class="fl"><span class="f_14px strong f_cen">管理资源</span></span><span class="fr"><img src="http://img2.topfo.com/member/images/biao_01.gif" align="absmiddle"/> <a href="http://www.topfo.com/help/demandrelease.shtml#16" target="_blank" class="publica-fbxq">需求发布规则</a></span></h1>
   <div class="search-1">需求筛选：<%--<input id="txtTitle" name="txtTitle" type="text" runat="server" value="aaa" />--%>
   <asp:TextBox runat="server" ID="txtTitle"></asp:TextBox>
   <asp:DropDownList ID="ddlInfoType" runat="server"></asp:DropDownList>
<input type="button" id="ccc"  class="btn-002" value="查询" onclick="tt();" />
</div>
        <h2>
        <ul>
         <li  class="btn_on"  id="sh_btn_1" onclick="Having(1,0,1);"><a href="#">审批通过（<span id="pass"></span>）</a></li>
...
<asp:Button runat="server" ID="btnDelete" OnClick="btnDelete_Click" OnClientClick="return SumDel();" CssClass="btn-002" Text="删除选中"/>
 <span id="spanb">
<asp:Button runat="server" ID="btnRefresh" OnClick="btnRefresh_Click" CssClass="btn-002" Text="刷新选中"/>
<asp:Button runat="server" ID="btnSetOverdue" OnClick="btnSetOverdue_Click" CssClass="btn-003" Text="放入已过期需求"/>
</span>
      </span>
