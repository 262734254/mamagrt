<%@ Page Language="C#" AutoEventWireup="true" CodeFile="NewsManageTest.aspx.cs" Inherits="news_NewsManageTest" %>
...
{
kong=shenhetype;
document.getElementById("NameID").value=shenhetype;
document.getElementById("txtPageID").value=pageindex;
...
}else if(shenhetype==4)
{
   ...
   $.ajax({type:"post",url:"News.ashx",data:"typeId="+daitype+"&username="+document.getElementById("username").value+"&loanstitle="+title+"&status="+shenhetype+"&type=1&pageIndex="+pageindex,success:function(msg) 
   { 
   document.getElementById("StatuID").innerHTML=msg;
   }
}); 
}
...
function Nopass()
{
 ...
 var title=document.getElementById("txtTitle").value;
 $.ajax({type:"post",url:"News.ashx",data:"typeId="+type+"&username="+document.getElementById("username").value+"&loanstitle="+title+"&type=3",success:function(msg) 
   { 
      var a=msg.split('$');
...
{
   $.ajax({type:"post",url:"News.ashx",data:"typeId="+daitype+"&loanstitle="+title+"&username="+document.getElementById("username").value+"&status="+shenhetype+"&type=2&pageIndex="+pageIndex,success:function(msg) 
   { 
...
function DelNav(id)
{
 ...
 $.ajax({type:"post",url:"DeleteNew.ashx",data:"Id="+id+"&type=1",success:function(msg) 
   { 
            ...
            var num=document.getElementById("txtPageID").value;
 ...
 var allid=document.getElementsByName("cbxSelect");
  ...
  if(str=="")
  {
   alert('请选择要删除的项目');
            ...
            var url="";
            var dd=0;
 $.ajax({type:"post",url:"DeleteNew.ashx",data:"str="+str+"&type=2",success:function(msg) 
   { 
               var num=document.getElementById("txtPageID").value;
            var strd=document.getElementById("NameID").value;
            if(num>0)
            {
             dd=parseInt(num);
             }
...
<input type="button" id="ccc"  class="btn-002" value="查询" onclick="search();" />
        ...
        <h2>
        <ul>
         <li  class="btn_on"  id="sh_btn_1" onclick="Having(1,0);"><a href="#">审批通过（<span id="pass"></span>）</a></li>
       <li  id="sh_btn_0" onclick="Having(0,0);"><a href="#" > 审核中（<span id="statu"></span>）</a></li>
        <li  id="sh_btn_2" onclick="Having(3,0);"> <a href="#">未通过审核（<span id="nopass"></span>）</a></li>
       <li  id="sh_btn_3" onclick="Having(4,0);"> <a href="#">已过期（<span id="Over"></span>）</a></li>
         </ul>
        </h2>
