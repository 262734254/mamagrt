...
var secure = (argc > 5) ? argv[5] : false; c
...
var isLogined = false;
var loginInfo = GetCookie("topfo.com_CKData");
if(loginInfo != null)
{    
...
function Logout()
{  
  location.href ='http://member.topfo.com/Logout.aspx?url='+location.href;
}

function CheckLogin_mini()
{  
	if (document.forms["form_mini"].txtLoginName.value.trim().length ==0)
	{
		alert("请输入用户名!");
		document.forms["form_mini"].txtLoginName.focus();
		return false;
	}
	if(document.forms["form_mini"].txtPassword.value.trim().length ==0)
	{
		alert("请输入密码!");
		document.forms["form_mini"].txtPassword.focus();
		return false;
	}
	else
	{	
...
function RequestByGet(URL,fcm)
{
   ...
   try {
    ...
    xmlhttp.SetRequestHeader ("SOAPAction","http://tempuri.org/GetUserData");
