<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="LoansManager.aspx.cs" Inherits="Publish_LoansManage" Title="Untitled Page" %>
...
}else if(shenhetype==4)
{
   ...
   $.ajax({type:"post",url:"PersonLons.ashx",data:"typeId="+daitype+"&username="+document.getElementById("ctl00_ContentPlaceHolder1_username").value+"&loanstitle="+title+"&status="+shenhetype+"&type=1&pageIndex="+pageindex,success:function(msg) 
   { 
...
function Nopass()
{
 ...
 var title=document.getElementById("ctl00_ContentPlaceHolder1_txtTitle").value;
 $.ajax({type:"post",url:"PersonLons.ashx",data:"typeId="+type+"&username="+document.getElementById("ctl00_ContentPlaceHolder1_username").value+"&loanstitle="+title+"&type=3",success:function(msg) 
   { 
      var a=msg.split('$');
...
{
   $.ajax({type:"post",url:"PersonLons.ashx",data:"typeId="+daitype+"&loanstitle="+title+"&username="+document.getElementById("ctl00_ContentPlaceHolder1_username").value+"&status="+shenhetype+"&type=2&pageIndex="+pageIndex,success:function(msg) 
   { 
...
function DelNav(id)
{
            var url="";
            var dd=0;
 $.ajax({type:"post",url:"DeleteLoans.ashx",data:"Id="+id+"&type=1",success:function(msg) 
   { 
...
function deleteall()
{
 ...
 var url="";
            var dd=0;
 $.ajax({type:"post",url:"DeleteLoans.ashx",data:"str="+strd+"&type=2",success:function(msg) 
   { 
             var num=document.getElementById("ctl00_ContentPlaceHolder1_txtPageID").value;
            var str=document.getElementById("ctl00_ContentPlaceHolder1_NameID").value;
            if(num>0)
...
function updaterefurbishtime(id)
{
   $.ajax({type:"post",url:"EnpriceLoans.ashx",data:"Id="+id,success:function(msg) 
   { 
    if(msg>0)
    {
    Having(document.getElementById("ctl00_ContentPlaceHolder1_NameID").value,document.getElementById("ctl00_ContentPlaceHolder1_txtPageID").value);
    }
   }});
}
function updateloans(ithisid,idd)
{
  if(ithisid==0)
  {
      url="loanupdatepersonss.aspx?str="+idd;
      window.location.href = url;
  }
  else if(ithisid==1)
  {
        url="LoansUpdateEnprise.aspx?str="+idd;
        window.location.href = url;
  }
}
 </script>
...
<div class="member-right">
    <div class="publication">
      <h1><span class="fl"><span class="f_14px strong f_cen">管理贷款</span></span><span class="fr"><img src="http://img2.topfo.com/member/images/biao_01.gif" align="absmiddle"/> <a href="#" class="publica-fbxq">需求发布规则</a></span></h1>
   ...
   <asp:DropDownList ID="ddlInfoType" runat="server">
       ...
       <asp:ListItem Value="0">个人贷款</asp:ListItem>
       <asp:ListItem Value="1">企业贷款</asp:ListItem>
   </asp:DropDownList>
