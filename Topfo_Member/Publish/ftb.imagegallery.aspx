...
private string[] AcceptedFileTypes = new string[] {"jpg","jpeg","jpe","gif","png","JPG","Gif"};
			...
			if (UploadFile.PostedFile.FileName.Trim() != "") {
				if (IsValidFileType(UploadFile.PostedFile.FileName)) {
					try {
                       ...
                       // UploadFileDestination = @"F:\Cases\";
						UploadFileDestination += CurrentImagesFolder.Value;
						UploadFileDestination += "\\";
                        string time = DateTime.Now.ToString("yyyyMMddhhmmss");
                        string[] img = UploadFileName.Split('.');
                        ViewState["name"] += time + "." + img[1] + ",";
                        string namess = ViewState["name"].ToString();
                        UploadFile.PostedFile.SaveAs(UploadFileDestination + time + "." + img[1]);
...
public void DeleteImage_OnClick(object sender, EventArgs e) {
	if (FileToDelete.Value != "" && FileToDelete.Value != "undefined") {
		try {
			string AppPath = HttpContext.Current.Request.PhysicalApplicationPath;
            //Response.Write(AppPath + CurrentImagesFolder.Value + "\\" + FileToDelete.Value);
...
private bool IsValidFileType(string FileName) {
	if(FileName!="bmp"&&FileName!="gif"&&FileName!="JPG"&&FileName!="GIF"&&FileName!="jpg")
				{
					return true;
				}
	return false;
...
public void DisplayImages() {
    string[] FilesArray = ViewState["name"].ToString().Split(',') ; 
	string AppPath = HttpContext.Current.Request.PhysicalApplicationPath;
	string AppUrl;
