...
<INPUT id="hfMaxFile" type="hidden" size="1" value="1024" name="hfMaxFile" runat="server"/>
...
<img src="<%=Img%>"  id="<%= this.ClientID %>_imgLoad" width="<%=ImgWidth%>" height="<%=ImgHeight%>"/>
	...
	UploaderFile.prototype.checkFile =  function(fileInput,strFileChecker)
	{
		...
		if(loc > 0)
		{
			...
			if((this.FileType).indexOf(extstr)>= 0 ) //(extstr == "jpg" || extstr == "png" || extstr == "gif")
			{
				...
				try
				{
					fileSize = fileCheckerEL.fileSize;
				}
				catch (e)
				{
					fileSize = 10;
