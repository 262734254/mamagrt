...
//初始化。
fileUploader.FileType = ".jpg|.gif|.png";//允许上传的文件类型，默认值是.jpg|.gif|.png
fileUploader.MaxPics = 4;//最大上传数,默认值为1.
fileUploader.IsCreateThumbnailImages = true;//是否创建缩略图，默认值是true
//以上参数设置在页面上设置也行。只需要设置MaxPics，其它使用默认值。
	
ArrayList picList = new ArrayList();//需要显示已经上传的图片
picList.Add(Pic1);  //Pic1,是图片相对于图片域名的相对路径
picList.Add(Pic2);
picList.Add(Pic3);
picList.Add(Pic4);
fileUploader.InitFiles = picList;
	
//保存,返回一个字符串数组,每个字符串是相对于图片域名的相对路径
	
string[] UploadImgFileName = fileUploader.SaveImages(theInfonType);
ArrayList errorMsg = fileUploader.ErrorMsg;//错误信息
...
using Tz888.BLL.Register;
    ...
    ///		上传图片用户控件:
    /// </summary>
public partial class Register_Control_FileUploader : System.Web.UI.UserControl
    {
        ...
        private bool m_IsCreateThumbnailImages = true;//是否创造缩略图
        ...
        private string _isUp = "1";//是否通过控件上传
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (UploadImgFileName.Length > 0)
        {
            ...
            //图片加载
            this.Img = ConfigurationManager.AppSettings["ImageDomain"].ToString() +"/"+ Pic1;
            this.ButtonName = "";
