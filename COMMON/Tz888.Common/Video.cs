...
namespace Tz888.Common
{
	...
	public class Video
	{
		...
		public Bitmap GetFrameFromVideo(string videoFile, double percentagePosition)
		{
			double streamLength;
		...
		/// <exception cref="ArgumentOutOfRangeException">在执行过程中可能抛出超出范围的异常，表示图像过大，不能正常生成-
		/// thrown if an invalid percentagePosition is passed</exception>
		public Bitmap GetFrameFromVideo(string videoFile, double percentagePosition, Size target)
		{
			double streamLength;
		...
		public void Save(string VideoFile, string LoginName, DateTime dt)
		{
			System.Drawing.Bitmap bmp = new Tz888.Common.Video().GetFrameFromVideo(VideoFile, 0.2d, scaleToFit(new System.Drawing.Size(160, 120), GetVideoSize(VideoFile)));
			string filepath = System.Web.HttpRuntime.AppDomainAppPath + "UploadFile\\" + LoginName + "\\";
			if(!System.IO.Directory.Exists(filepath))
			{
				System.IO.Directory.CreateDirectory(filepath);	
			}
			bmp.Save(System.Web.HttpRuntime.AppDomainAppPath + "UploadFile\\" + LoginName + "\\" + dt.ToLocalTime().ToString().Replace("-", "").Replace(":", "").Replace(" ", "").Replace("/", "") + new Random().Next(1000).ToString() + ".jpg", System.Drawing.Imaging.ImageFormat.Jpeg);
			bmp.Dispose();
		}
		...
		/// <exception cref="ArgumentOutOfRangeException">在执行过程中可能抛出超出范围的异常，表示图像过大，不能正常生成-
		/// thrown if an invalid percentagePosition is passed</exception>
		public Bitmap GetFrameFromVideo(string videoFile, double percentagePosition, out double streamLength, Size target)
		{
			...
			try
			{
				...
				if (openVideoStream(videoFile, out mediaDet, out mediaType))
				{
                    ...
                    unsafe
					{
						...
						Bitmap bmp = new Bitmap(target.Width, target.Height, target.Height * 4, System.Drawing.Imaging.PixelFormat.Format24bppRgb, new IntPtr(frameBuffer2 + bmpinfoheaderSize));
		...
		#region 吸取一桢从视频文件里面，并且将其保存为bmp格式到特定目录
		/// <summary>
		...
		/// <exception cref="ArgumentOutOfRangeException">在执行过程中可能抛出超出范围的异常，表示图像过大，不能正常生成-
		/// thrown if an invalid percentagePosition is passed</exception>
		public void SaveFrameFromVideo(string videoFile, double percentagePosition, string outputBitmapFile)
		{
			double streamLength;
		...
		/// <exception cref="ArgumentOutOfRangeException">在执行过程中可能抛出超出范围的异常，表示图像过大，不能正常生成-
		/// thrown if an invalid percentagePosition is passed</exception>
		public void SaveFrameFromVideo(string videoFile, double percentagePosition, string outputBitmapFile, Size target)
		{
			double streamLength;
		...
		/// <exception cref="ArgumentOutOfRangeException">在执行过程中可能抛出超出范围的异常，表示图像过大，不能正常生成-
		/// thrown if an invalid percentagePosition is passed</exception>
		public void SaveFrameFromVideo(string videoFile, double percentagePosition, string outputBitmapFile, out double streamLength, Size target)
		{
			if (percentagePosition > 1 || percentagePosition < 0)
				throw new ArgumentOutOfRangeException("percentagePosition", percentagePosition, "Valid range is 0.0 .. 1.0");
		...
		public Size GetVideoSize(string videoFile)
		{
			MediaDetClass mediaDet;
			_AMMediaType mediaType;
			if (openVideoStream(videoFile, out mediaDet, out mediaType))
		...
		/// <returns>桢大小</returns>
		private Size getVideoSize(_AMMediaType mediaType)
		{
		...
		private Size scaleToFit(Size target, Size original)
		{
		...
		/// <param name="original">源大小</param>
		/// <returns>实际大小</returns>
		private Size scaleToFitSmart(Size target, Size original)
		{
		...
		/// <returns></returns>
		private string getErrorMsg(uint errorCode)
		{
	...
	{
		#region 频信息头结构用来描述视频图像的bitmap和颜色信息
		/// <summary>
		...
		#region bitmap信息头结构用来描述bitmap图像的维度和颜色信息
		/// <summary>
